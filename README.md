...
To run all the examples in the paper, source the file `00-run-all-examples.R`. The top of the file contains a command for installing all necessary packages, wrapped in a `if (FALSE)` statement so it won't be run on sourcing, but you can run it interactively if you need to.
